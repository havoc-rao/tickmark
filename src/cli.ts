...
用法:
  ...
  ${bin} ide [list]          列出本机可用的（预设）IDE 及当前默认
  ${bin} ide set <cmd>       设置默认 IDE（写入 ~/.config/tickmark/config.json）
...
功能:
  ...
  • 自动用 IDE 打开预览（可用 ${bin} ide 自选 IDE，见下）
...
IDE 选择（优先级高→低）:
  ...
  3. 默认 IDE: ${bin} ide set cursor           （写入 ~/.config/tickmark/config.json）
  4. 自动探测: buddycn → code → cursor → …（${bin} ide list 查看本机已装）

示例:
  ${bin} ide                                  # 查看已装 IDE 与当前默认
  ${bin} ide set buddycn                      # 默认用 CodeBuddy (CN) 打开
