/**
 * TickMark CLI — 页面交互脚本（业务逻辑）
 *
 * 本文件与 utils.js / templates.js 由 scripts/build-media.js 合并进同一 IIFE
 * 共享作用域（合并顺序：utils → templates → preview），函数声明 / var 直接
 * 可见，无导入导出。
 *
 * 依赖：
 * - utils.js：api / closest / cellText / tmPopover / showToast /
 *              getStored / setStored / themeSystemDark /
 *              scrollTopOf / smoothScrollTo / findScroller
 * - templates.js：TM_ICON_FILTER / TM_FPOP_HTML / TM_AC_POP_HTML /
 *                 TM_HISTORY_GROUP_HTML / TM_OUTLINE_HTML / TM_THEME_POP_HTML
 *
 * 职责：
 * - checkbox change → POST /api/toggle → 成功保持勾选，失败回滚 + toast
 * - Sync 按钮 → GET /api/html 拉最新整页 → DOMParser 替换 .markdown-body
 * - 代码块复制按钮
 * - 表格列过滤（聚合统计 / 多选 / 全选 / 反选）
 * - 表格列编辑（新增列 / 删除列 / 单元格填写回写 / 拖拽换列）
 * - Checkbox 列单元格、修改历史（撤销/重做/重置）、编辑模式
 * - 大纲 TOC、主题切换
 */
  ...
  fetch(api('/api/toggle'), {
    ...
    .then(function (r) {
      if (r.status === 200 && r.data.ok) {
        ...
        showToast(r.data.reason || I18N.t('err.toggle', r.status), true);
      }
    })
    .catch(function (err) {
      target.checked = !checked; // 网络失败回滚
      console.error('[tickmark] fetch failed:', err);
      showToast(I18N.t('err.connect', API || '?'), true);
    });
});
...
function reRender(okMsg) {
  fetch(api('/api/html'))
    ...
    .catch(function () {
      showToast(I18N.t('toast.syncFail'), true);
    });
}
...
/** 空单元格显示灰色占位（(空) / (empty)），提升表格可读性 */
function markEmptyCells(table) {
  var tds = table.querySelectorAll('td');
  var placeholder = I18N.t('common.empty');
...
function getPopup() {
  if (!filterPopover) {
    ...
    pop.innerHTML = TM_FPOP_HTML;
    // 不 append 到 body：由 popover 挂载到当前表格的定位容器
    filterPopover = tmPopover({ content: pop, maxWidth: 310, maxHeight: 400, gap: 6 });
    localeFilterPopup();   // 设置 placeholder / 按钮文本（i18n 切换时也会再调）
  }
  return filterPopover.el;
}

/** 给列过滤弹层绑定静态 i18n 文案（placeholder + 按钮）；面板首次创建 + 语言切换时调用 */
function localeFilterPopup() {
  var pop = filterPopover && filterPopover.el;
  if (!pop) return;
  pop.querySelector('.tm-fpop-master').title = I18N.t('common.confirm');
  pop.querySelector('.tm-fpop-search').placeholder = I18N.t('filter.searchPh');
  var acts = pop.querySelectorAll('.tm-fpop-actions [data-act]');
  var actKeys = { all: 'filter.all', invert: 'filter.invert', none: 'filter.none' };
  for (var i = 0; i < acts.length; i++) {
    var k = acts[i].getAttribute('data-act');
    if (actKeys[k]) acts[i].textContent = I18N.t(actKeys[k]);
  }
}
...
function renderFilterBar(m) {
  ...
  for (c = 0; c < m.cols.length; c++) {
    ...
    nm.textContent = m.cols[c] || I18N.t('common.noHeader');
  ...
  sum.textContent = I18N.t('filter.rowCount', visible, m.rows.length);
  opRow.appendChild(sum);
  var clearBtn = document.createElement('button');
  clearBtn.type = 'button';
  clearBtn.className = 'tm-tbl-clear';
  clearBtn.textContent = I18N.t('filter.clear');
  if (!hasActive) clearBtn.disabled = true;
  opRow.appendChild(clearBtn);
  var addColBtn = document.createElement('button');
  addColBtn.type = 'button';
  addColBtn.className = 'tm-tbl-addcol';
  addColBtn.textContent = I18N.t('filter.addCol');
  addColBtn.title = I18N.t('filter.addCol.title');
...
function renderFilterList(m) {
  ...
  if (!keys.length) {
    ...
    empty.textContent = I18N.t('filter.empty');
  ...
  return {
    ...
    sumText: keys.length ? I18N.t('filter.sum', sel.size, keys.length) : I18N.t('filter.sumNone'),
    titleText: m.cols[col] || I18N.t('common.noHeader'),
  };
}
...
function getAddColPopup() {
  ...
  // 每次访问都保证最新文案（i18n 切换后会调用 repaint）
  localeAddColPopup();
 ...
 * - 一致的视觉（与主题/大纲弹层相同的圆角 + 阴影 + 强调色）
 * - 选中项前打 ✓；选项点击立即关闭
 * - 通过 I18N.t() 实时计算 label，i18n 切换时 repaint() 即可
 * - API：
 *     ref.cur        当前选中 value
 *     ref.options    [{ value, label }]
 *     ref.setOptions(opts) / setValue(v) / repaint() / onPick(v, opt)
 *     ref.open() / ref.close() / ref.toggle()
 */
function mountChoose(container) {
  if (!container) return null;
  if (container.__tmChooseRef) return container.__tmChooseRef;
  container.classList.add('tm-choose-init');
  container.innerHTML =
    '<button type="button" class="tm-choose-trigger">' +
    '<span class="tm-choose-label"></span>' +
    '</button>' +
    '<div class="tm-choose-panel" role="listbox"></div>';
  var trigger = container.querySelector('.tm-choose-trigger');
  var panel = container.querySelector('.tm-choose-panel');
  var ref = {
    options: [],
    cur: null,
    onPick: null,
    ...
    toggle: function () {
      if (container.classList.contains('open')) ref.close();
      else ref.open();
    },
    ...
    repaint: function () {
      ...
      // trigger 显示当前选中 label
      var cur = null;
      for (var i = 0; i < opts.length; i++) {
        if (opts[i].value === ref.cur) { cur = opts[i]; break; }
      }
      var lab = container.querySelector('.tm-choose-label');
      if (lab) lab.textContent = cur ? cur.label : '';
      // panel 重新渲染
      var panelEl = container.querySelector('.tm-choose-panel');
      panelEl.innerHTML = '';
      for (var j = 0; j < opts.length; j++) {
        (function (o) {
          var opt = document.createElement('div');
          opt.className = 'tm-choose-opt';
          opt.setAttribute('role', 'option');
          opt.setAttribute('data-value', String(o.value));
          if (o.value === ref.cur) opt.classList.add('selected');
          var check = document.createElement('span');
          check.className = 'tm-choose-check';
          check.textContent = o.value === ref.cur ? '\u2713' : '';
          var label = document.createElement('span');
          label.className = 'tm-choose-opt-label';
          ...
          opt.addEventListener('click', function (e) {
            ...
            ref.close();
            ref.repaint();
            if (typeof ref.onPick === 'function') {
              try { ref.onPick(o.value, o); } catch (err) { /* ignore */ }
            }
          });
          panelEl.appendChild(opt);
        })(opts[j]);
      }
      // 打开态重新打开（repaint 后状态保留）
  ...
  // panel 内点击不要冒泡（避免触发外层弹层关闭）
  panel.addEventListener('click', function (e) { e.stopPropagation(); });
  container.__tmChooseRef = ref;
  return ref;
}

/** 应用最新文案（label / placeholder / button）；i18n 切换时会再调用一次 */
function localeAddColPopup() {
  var pop = addColPopover && addColPopover.el;
  if (!pop) return;
  pop.querySelector('.tm-ac-title').textContent = I18N.t('addCol.title');
  pop.querySelector('.tm-ac-name').placeholder = I18N.t('addCol.namePh');
  // 三处 label（type / cb / pos）
  var labels = pop.querySelectorAll('.tm-ac-type-row > .tm-ac-label, .tm-ac-cb-row > .tm-ac-label, .tm-ac-pos-row > .tm-ac-label');
  var labelKeys = ['addCol.type', 'addCol.cbDefault', 'addCol.pos'];
  for (var i = 0; i < labels.length && i < labelKeys.length; i++) {
    labels[i].textContent = I18N.t(labelKeys[i]);
  }
  // 列类型
  var typeBtns = pop.querySelectorAll('.tm-ac-type');
  for (var t = 0; t < typeBtns.length; t++) {
    typeBtns[t].textContent = typeBtns[t].getAttribute('data-type') === 'text'
      ? I18N.t('addCol.type.text')
      : I18N.t('addCol.type.checkbox');
  }
  // 复选框默认按钮
  var cbBtns = pop.querySelectorAll('.tm-ac-cb-default');
  for (var c = 0; c < cbBtns.length; c++) {
    cbBtns[c].textContent = cbBtns[c].getAttribute('data-val') === '[ ]'
      ? I18N.t('addCol.cbDefault.unchecked')
      : I18N.t('addCol.cbDefault.checked');
  }
  pop.querySelector('.tm-ac-fill').placeholder = I18N.t('addCol.fillPh');
  // 之前 / 之后
  var sideBtns = pop.querySelectorAll('.tm-ac-pos-side');
  for (var s = 0; s < sideBtns.length; s++) {
    sideBtns[s].textContent = sideBtns[s].getAttribute('data-side') === 'after'
      ? I18N.t('addCol.pos.after')
      : I18N.t('addCol.pos.before');
  }
  pop.querySelector('.tm-ac-close').title = I18N.t('common.cancel');
  pop.querySelector('.tm-ac-cancel').textContent = I18N.t('common.cancel');
  pop.querySelector('.tm-ac-ok').textContent = I18N.t('common.confirm');
  // 自定义 choose 组件 repaint（保证 label 语言切换）
  var chooseHost = pop.querySelector('.tm-ac-pos-choose');
  if (chooseHost && chooseHost.__tmChooseRef) chooseHost.__tmChooseRef.repaint();
}
...
function openAddColPopup(table, colIndex, anchorFn, wrap) {
  ...
  addColCbVal = '[ ]';
  syncAddColCbButtons();
  // 自定义 choose 组件：替换原生 select，选项 = 末尾 + 每个列名
  var model = table.__tmModel;
  var cols = model ? model.cols : [];
  var choose = mountChoose(pop.querySelector('.tm-ac-pos-choose'));
  if (choose) {
    var opts = [{ value: 'end', label: I18N.t('addCol.pos.end') }];
    for (var i = 0; i < cols.length; i++) {
      opts.push({ value: String(i), label: cols[i] || I18N.t('common.noHeader') });
    }
    choose.setOptions(opts);
    choose.setValue('end');
  }
  // 默认「之后」
  var sideBtns = pop.querySelectorAll('.tm-ac-pos-side');
...
function confirmAddCol() {
  ...
  var chooseHost = pop.querySelector('.tm-ac-pos-choose');
  var chooseRef = chooseHost && chooseHost.__tmChooseRef;
  var posVal = chooseRef ? String(chooseRef.cur) : 'end';
  var colIndex;
  if (posVal === 'end') {
  ...
  fetch(api('/api/table/add-column'), {
    ...
    .then(function (r) {
      if (r.status === 200 && r.data.ok) {
        ...
        closeAddColPopup();
        showToast(I18N.t('toast.added', r.data.name), false);
        refreshHistoryButtons();
      } else {
        showToast(r.data.reason || I18N.t('err.addCol', r.status), true);
      }
    })
    .catch(function (err) {
      console.error('[tickmark] add column failed:', err);
      showToast(I18N.t('err.connectCol'), true);
    });
}
...
function localInsertColumn(table, idx, name, fillValue) {
  ...
  for (var ii = 0; ii < tbody.rows.length; ii++) {
    ...
    if (isCb) {
      ...
      td.title = I18N.t('cell.editTitle');
      if (fillValue) {
...
function addThGrip(th) {
  ...
  grip.title = I18N.t('th.grip.title');
  grip.textContent = '⋮⋮';
  ...
  fetch(api('/api/table/move-column'), {
    ...
    .then(function (r) {
      if (r.status === 200 && r.data.ok) {
        ...
        showToast(I18N.t('toast.swapped'), false);
        refreshHistoryButtons();
      } else {
        showToast(r.data.reason || I18N.t('err.moveCol', r.status), true);
      }
    })
    .catch(function (err) {
...
function confirmDeleteColumn(btn) {
  ...
  var colName = model.cols[col] || I18N.t('common.noHeader');
  fetch(api('/api/table/delete-column'), {
    ...
    .then(function (r) {
      if (r.status === 200 && r.data.ok) {
        ...
        showToast(I18N.t('toast.deleted', colName), false);
        refreshHistoryButtons();
      } else {
        showToast(r.data.reason || I18N.t('err.delCol', r.status), true);
      }
    })
    .catch(function (err) {
  ...
  fetch(api('/api/table/set-cell'), {
    ...
    .then(function (r) {
      if (r.status === 200 && r.data.ok) {
        ...
        showToast(r.data.reason || I18N.t('err.setCell', r.status), true);
      }
    })
    .catch(function (err) {
      td.textContent = oldValue;
...
function ensureHistoryButtons() {
  ...
  group.innerHTML = TM_HISTORY_GROUP_HTML;
  ...
  localeHistoryButtons();
  return historyBtns;
}

/** 历史按钮文本 + title（依赖 i18n；语言切换时 rebuild 调用） */
function localeHistoryButtons() {
  if (!historyBtns) return;
  historyBtns.undo.textContent = I18N.t('hist.undo');
  historyBtns.redo.textContent = I18N.t('hist.redo');
  historyBtns.reset.textContent = I18N.t('hist.reset');
  historyBtns.undo.title = I18N.t('hist.undo.title');
  historyBtns.redo.title = I18N.t('hist.redo.title');
  historyBtns.reset.title = I18N.t('hist.reset.title');
}
...
function resetClickHandler(e) {
  ...
  if (btn.classList.contains('confirming')) {
    btn.classList.remove('confirming');
    btn.textContent = I18N.t('hist.reset');
    clearTimeout(btn.__tmConfirmTimer);
    doHistory('reset');
  } else {
    btn.classList.add('confirming');
    btn.textContent = I18N.t('hist.reset.confirm');
    clearTimeout(btn.__tmConfirmTimer);
...
function doHistory(act) {
  fetch(api('/api/history/' + act), { method: 'POST' })
      ...
      .then(function (r) {
        if (r.status === 200 && r.data.ok) {
          var msg = act === 'reset' ? I18N.t('toast.reset') : (act === 'undo' ? I18N.t('toast.undo') : I18N.t('toast.redo'));
          reRender(msg);
        } else {
  ...
  fetch(api('/api/history/status'))
    ...
    .then(function (s) {
      ...
      if (resetBtn.classList.contains('confirming')) {
        ...
        resetBtn.textContent = I18N.t('hist.reset');
...
function ensureOutlineToggle() {
  ...
  btn.type = 'button';
  btn.title = I18N.t('toolbar.outline.title');
  btn.innerHTML = '<span class="tm-toc-ic" aria-hidden="true">☰</span> <span class="tm-outline-label"></span>';
  ...
  tocToggle = btn;
  tocWrap = wrap;
  localeOutline();   // 给按钮填充当前语言 label
}

/** 大纲按钮 label / 面板标题 / close title 都用 i18n；面板首次创建 + 语言切换时调用 */
function localeOutline() {
  if (tocToggle) {
    var lab = tocToggle.querySelector('.tm-outline-label');
    if (lab) lab.textContent = I18N.t('toolbar.outline');
  }
  if (tocPanel) {
    var ttl = tocPanel.querySelector('.tm-toc-title');
    if (ttl) ttl.textContent = I18N.t('outline.title');
    var closeBtn = tocPanel.querySelector('.tm-toc-close');
    if (closeBtn) closeBtn.title = I18N.t('outline.close');
    // 大纲列表里"暂无标题"为空态文案（重建触发 buildOutline 时已经用 i18n.t 填一次）
  }
}
...
function createOutlinePanel() {
  ...
  panel.setAttribute('aria-label', I18N.t('outline.title'));
  panel.innerHTML = TM_OUTLINE_HTML;
...
function buildOutline() {
  ...
  for (var i = 0; i < headings.length; i++) {
    (function (h) {
      ...
      var txt = (h.textContent || '').replace(/\s+/g, ' ').trim() || I18N.t('outline.emptyHeading');
      a.textContent = txt;
      a.title = txt;
  ...
  if (!tocItems.length) {
    ...
    empty.textContent = I18N.t('outline.emptyTitle');
...
// ---- 主题切换：外观（浅色 / 深色 / 跟随系统）+ 强调色 + 语言 ----
var THEME_STORAGE_KEY = 'tickmark.theme';
var ACCENT_STORAGE_KEY = 'tickmark.accent';
/** 强调色预设：label 通过 I18N.t('theme.opt.<id>') 在 buildThemePop 时填充 */
var ACCENT_PRESETS = [
  { id: 'blue', color: '#0969da' },
  { id: 'green', color: '#1a7f37' },
  { id: 'purple', color: '#8250df' },
  { id: 'orange', color: '#bc4c00' },
  { id: 'rose', color: '#cf222e' },
];
var themeWrap = null;
...
function buildThemePop() {
  ...
  pop.setAttribute('aria-label', 'Theme settings');
  pop.innerHTML = TM_THEME_POP_HTML;

  // 外观（浅色 / 深色 / 跟随系统）
  var themeBox = pop.querySelector('[data-group="theme"]');
  var themeIds = ['light', 'dark', 'system'];
  for (var i = 0; i < themeIds.length; i++) {
    ...
    b.textContent = I18N.t('theme.opt.' + id);
    themeBox.appendChild(b);
  }

  // 强调色
  var accentBox = pop.querySelector('[data-group="accent"]');
  for (var j = 0; j < ACCENT_PRESETS.length; j++) {
    var a = ACCENT_PRESETS[j];
    var ab = document.createElement('button');
    ab.type = 'button';
    ab.className = 'tm-theme-opt tm-theme-accent';
    ab.setAttribute('data-accent-opt', a.id);
    ab.setAttribute('data-label', a.id);
    ab.innerHTML = '<span class="tm-dot" style="background:' + a.color + '"></span> <span class="tm-theme-accent-label"></span>';
    accentBox.appendChild(ab);
  }

  // 语言（zh-CN / en-US）
  var langBox = pop.querySelector('[data-group="lang"]');
  var langs = I18N.langs();
  for (var langId in langs) {
    if (!langs.hasOwnProperty(langId)) continue;
    var lb = document.createElement('button');
    lb.type = 'button';
    lb.className = 'tm-theme-opt tm-theme-lang';
    lb.setAttribute('data-lang-opt', langId);
    lb.textContent = langs[langId].label;
    langBox.appendChild(lb);
  }
  ...
  pop.addEventListener('click', function (e) {
    ...
    var ln = t.closest ? t.closest('[data-lang-opt]') : null;
    if (ln) {
      I18N.setLang(ln.getAttribute('data-lang-opt'));
      // 语言切换后回调会调用 syncThemeMenu / 等所有 repaint
      syncThemeMenu();
    }
  });
  themePop = pop;
  localeTheme();   // 设置 section title / close title / accent 标签
  return pop;
}

/** 主题菜单静态 i18n 文本：title / 三个 section / close / accent label */
function localeTheme() {
  var pop = themePop || buildThemePop();
  if (!pop) return;
  pop.setAttribute('aria-label', I18N.t('theme.title'));
  pop.querySelector('.tm-theme-title').textContent = I18N.t('theme.title');
  pop.querySelector('.tm-theme-close').title = I18N.t('common.close');
  pop.querySelector('.tm-theme-close').setAttribute('aria-label', I18N.t('common.close'));
  var secs = pop.querySelectorAll('.tm-theme-sec');
  var secKeys = { appearance: 'theme.section.appearance', accent: 'theme.section.accent', lang: 'theme.section.lang' };
  for (var i = 0; i < secs.length; i++) {
    var k = secs[i].getAttribute('data-sec');
    if (secKeys[k]) secs[i].textContent = I18N.t(secKeys[k]);
  }
  // 强调色按钮：label 实时从 STRINGS 取
  var accentBtns = pop.querySelectorAll('[data-accent-opt]');
  for (var a = 0; a < accentBtns.length; a++) {
    var lbl = accentBtns[a].querySelector('.tm-theme-accent-label');
    if (lbl) lbl.textContent = I18N.t('theme.opt.' + accentBtns[a].getAttribute('data-label'));
  }
  // 外观按钮：label 也走 i18n（首次构建时已填过；切换时再补一次保持一致）
  var themeBtns = pop.querySelectorAll('[data-theme-opt]');
  for (var t = 0; t < themeBtns.length; t++) {
    themeBtns[t].textContent = I18N.t('theme.opt.' + themeBtns[t].getAttribute('data-theme-opt'));
  }
}
...
function syncThemeMenu() {
  var pop = buildThemePop();
  if (!pop) return;
  // 切语言后所有静态文本重写（按钮 / section 标题 / accent label）
  localeTheme();
  ...
  var lngs = pop.querySelectorAll('[data-lang-opt]');
  for (var k = 0; k < lngs.length; k++) {
    lngs[k].classList.toggle('active', lngs[k].getAttribute('data-lang-opt') === lang);
  }
}
...
function initTheme() {
  ...
  // i18n：scan 所有 [data-i18n-key] 写文字 + [data-i18n-title] 写 tooltip
  initToolbari18n();

  // 语言切换 → 重建各面板静态文本
  I18N.subscribe(function () {
    initToolbari18n();
    if (historyBtns) localeHistoryButtons();
    if (themePop) localeTheme();
    if (filterPopover) localeFilterPopup();
    localeOutline();
    // 重新挂表格过滤栏（按钮文案已 i18n 化）
    rebuildAllFilterBars();
    // 关闭中的弹层下次打开时由 getXxxPopup() 内 localeXxx() 重新填写
  });
}

/**
 * 扫描页面上静态 i18n 占位（page.ts 模板里渲染的）并写文字：
 * - [data-i18n-key]   → textContent = I18N.t(key)
 * - [data-i18n-title] → title       = I18N.t(key)
 * - [data-i18n-aria]  → setAttribute('aria-label', ...)
 */
function initToolbari18n() {
  var k1 = document.querySelectorAll('[data-i18n-key]');
  for (var i = 0; i < k1.length; i++) {
    var key = k1[i].getAttribute('data-i18n-key');
    k1[i].textContent = I18N.t(key);
  }
  var t1 = document.querySelectorAll('[data-i18n-title]');
  for (var j = 0; j < t1.length; j++) {
    t1[j].title = I18N.t(t1[j].getAttribute('data-i18n-title'));
  }
  var a1 = document.querySelectorAll('[data-i18n-aria]');
  for (var k = 0; k < a1.length; k++) {
    a1[k].setAttribute('aria-label', I18N.t(a1[k].getAttribute('data-i18n-aria')));
  }
}

/** 重新渲染所有表格过滤栏（按钮文案 i18n 化） */
function rebuildAllFilterBars() {
  var tables = document.querySelectorAll('.markdown-body table');
  for (var i = 0; i < tables.length; i++) {
    var m = tables[i].__tmModel;
    if (m) mountFilterBar(m);
  }
}
